...
object Free {
  implicit def freeFunctorInstance[F[_]] = new Functor[({type E[X] = Free[F, X]})#E] {
    override def fmap[A, B](a: Free[F, A])(fx: A => B): Free[F, B] = FlatMap(a, (in: A) => Return(fx(in)))
  }
  ...
  def foldF[F[_], M[_], A](a:Free[F, A])(trans:Natural[F, M])(implicit f:Functor[M], m:Monad[M]):M[A] = a match {
    case Return(aValue) => m.pure(aValue)
    case FlatMap(aValue, fxFunc) => m.flatMap(foldF(aValue)(trans))(am => foldF(fxFunc(am))(trans))
    case LiftF(fnValue) => trans.transform(fnValue)
  }
}